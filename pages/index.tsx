...
const lift = { whileHover: { y: -6, transition: { duration: 0.2, ease: EASE_OUT } } };
...
export default function Home() {
  ...
  const breedSuggestions: { [key: string]: string[] } = {
    ...
    other: ['Parrot', 'Lovebird', 'Cockatiel', 'Goldfish', 'Betta Fish', 'Turtle', 'Hamster', 'Guinea Pig', 'Rabbit', 'Aquarium Fish'],
  };
  ...
  const handleInputChange = (e: any) => {
    ...
    if (name === 'petType') setFormData(prev => ({ ...prev, [name]: value, breed: '' }));
  ...
  const inputClass = (field: string) =>
    `w-full px-4 py-3 rounded-xl border text-sm transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-[#FFD447]/30 ${
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="Pet.Ra offers monthly pet subscription plans, verified pet finder services, grooming, vet care, and premium pet supplies in Kochi. Find your perfect pet from trusted breeders in 24 hours." />
      ...
      <div className="min-h-screen bg-white font-sans">
          ...
          <div className="max-w-6xl mx-auto px-6 sm:px-10 py-5 flex items-center justify-between">
            ...
            <div className="flex items-center gap-1">
              {status === 'authenticated' ? (
                <>
                  <Link href="/dashboard"><span className={`px-3 py-2 text-sm font-medium transition-colors ${scrolled ? 'text-black/60 hover:text-black' : 'text-white/60 hover:text-white'}`}>Dashboard</span></Link>
                  <Link href="/subscriptions"><span className={`px-3 py-2 text-sm font-medium transition-colors ${scrolled ? 'text-black/60 hover:text-black' : 'text-white/60 hover:text-white'}`}>Plans</span></Link>
                  <button onClick={() => signOut({ callbackUrl: '/' })} className={`px-3 py-2 text-sm font-medium transition-colors ${scrolled ? 'text-black/40 hover:text-black' : 'text-white/40 hover:text-white'}`}>Sign Out</button>
                </>
              ) : (
                <Link href="/auth/signin"><span className={`px-3 py-2 text-sm font-medium transition-colors ${scrolled ? 'text-black/60 hover:text-black' : 'text-white/60 hover:text-white'}`}>Sign In</span></Link>
              )}
              <button onClick={scrollToPetFinder} className="ml-4 px-5 py-2 bg-[#FFD447] text-[#0B0C1E] text-sm font-bold rounded-full hover:bg-[#F5C800] transition-all shadow-lg shadow-[#FFD447]/20">
                Find a Pet →
              </button>
            </div>
          </div>
        ...
        {/* ── HERO ── */}
        <section className="relative min-h-screen bg-[#0B0C1E] flex items-center overflow-hidden pt-20">
          {/* Background layers */}
          <div className="absolute inset-0 dot-grid opacity-50" />
          <div className="absolute top-1/4 right-1/3 w-[500px] h-[500px] bg-[#30358B]/25 rounded-full blur-[120px] pointer-events-none" />
          <div className="absolute bottom-1/4 left-1/4 w-[300px] h-[300px] bg-[#FFD447]/8 rounded-full blur-[80px] pointer-events-none" />
            ...
            <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
              ...
              <div>
                <motion.div {...stagger(0)} className="flex items-center gap-2 mb-8">
                  ...
                  <span className="font-mono text-[10px] tracking-[0.35em] uppercase text-[#FFD447]/60">
                ...
                <motion.h1
                  {...stagger(1)}
                  className="text-[clamp(3rem,6.5vw,5.8rem)] font-black text-white leading-[1.0] tracking-[-0.025em] mb-6"
                >
                  Find your<br />perfect<br />companion.
                </motion.h1>

                <motion.p {...stagger(2)} className="text-base text-white/50 leading-relaxed mb-10 max-w-sm">
                  Every pet comes from screened, ethical breeders with verified health records. Your perfect match, in 24 hours.
                </motion.p>

                <motion.div {...stagger(3)} className="flex gap-10 mb-10">
                  {[
                    ...
                    <div key={label}>
                      <div className="font-mono text-2xl font-bold text-[#FFD447]">{num}</div>
                      <div className="font-mono text-[9px] tracking-[0.15em] uppercase text-white/30 mt-1">{label}</div>
                    </div>
                  ))}
                </motion.div>
                ...
                <motion.div {...stagger(4)} className="flex flex-col sm:flex-row gap-3">
                  <button
                    ...
                    className="inline-flex items-center gap-2 px-8 py-4 bg-[#FFD447] text-[#0B0C1E] font-bold rounded-full text-sm hover:bg-[#F5C800] transition-all shadow-2xl shadow-[#FFD447]/25 hover:shadow-[#FFD447]/40 hover:-translate-y-0.5"
                  >
                    Start Pet Finder
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-2 px-8 py-4 border border-white/10 text-white/60 font-medium rounded-full text-sm hover:border-white/25 hover:text-white/80 transition-all"
                  >
                    View Care Plans
                  </button>
                </motion.div>
              ...
              >
                ...
                <div className="relative rounded-2xl overflow-hidden ring-1 ring-white/10">
                  ...
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-[#0B0C1E]/50 via-transparent to-transparent" />

                  {/* Floating badge */}
                  <motion.div
                    animate={{ y: [0, -8, 0] }}
                    transition={{ duration: 3.5, repeat: Infinity, ease: 'easeInOut' }}
                    className="absolute bottom-5 left-5 right-5 bg-[#0B0C1E]/70 backdrop-blur-md border border-white/15 rounded-xl px-5 py-4"
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 bg-[#FFD447] rounded-full flex items-center justify-center flex-shrink-0">
                        <Check size={17} weight="bold" className="text-[#0B0C1E]" />
                      </div>
                      <div>
                      ...
                      <div className="ml-auto text-right">
                        <div className="text-[#FFD447] font-mono text-xs font-bold">24h</div>
                        <div className="text-white/30 font-mono text-[9px] uppercase tracking-widest">delivery</div>
                      </div>
                    </div>
                  </motion.div>
                  ...
                  <motion.div
                    ...
                    className="absolute top-5 right-5 bg-[#FFD447] rounded-xl px-3 py-2 shadow-xl"
                  >
                    <div className="font-mono text-[9px] tracking-widest uppercase text-[#0B0C1E] font-bold">200+</div>
                    <div className="font-mono text-[8px] tracking-wide text-[#0B0C1E]/60">Families Matched</div>
                  </motion.div>
                </div>
              </motion.div>
        ...
        <section className="py-28 lg:py-36 bg-white">
          <div className="max-w-6xl mx-auto px-6 sm:px-10">
            ...
            <motion.div {...fadeUp} className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6 pb-10 mb-12 border-b border-black/[0.07]">
              <div>
            ...
            <div className="grid md:grid-cols-3 gap-5">
              {[
                {
                  tag: '2–3 min', badge: 'Recommended', tagColor: 'bg-violet-100 text-violet-700',
                ...
                {
                  tag: '30 sec', badge: null, tagColor: 'bg-emerald-100 text-emerald-700',
                ...
                {
                  tag: 'Instant', badge: null, tagColor: 'bg-sky-100 text-sky-700',
                ...
                <motion.div
                  ...
                  {...stagger(i)}
                  {...lift}
                  className={`relative flex flex-col rounded-2xl p-8 border transition-shadow duration-300 hover:shadow-2xl ${
                    card.dark ? 'bg-[#0B0C1E] border-white/[0.08]' : 'bg-white border-black/[0.07]'
                  }`}
                >
                  {card.badge && (
                    <span className="absolute top-5 right-5 px-3 py-1 bg-[#FFD447] text-[#0B0C1E] font-mono text-[9px] tracking-[0.2em] uppercase rounded-full">
                      {card.badge}
                    </span>
                  )}

                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-5 ${card.dark ? 'bg-[#FFD447]' : 'bg-gray-100'}`}>
                    <div className={card.dark ? 'text-[#0B0C1E]' : 'text-[#0B0C1E]'}>{card.icon}</div>
                  </div>

                  <span className={`inline-block px-2.5 py-1 rounded-full text-xs font-bold mb-4 w-fit ${card.tagColor}`}>{card.tag}</span>

                  <h3 className={`text-xl font-black mb-3 ${card.dark ? 'text-white' : 'text-[#0B0C1E]'}`}>{card.title}</h3>
                  <p className={`text-sm leading-relaxed mb-6 flex-grow ${card.dark ? 'text-white/50' : 'text-gray-500'}`}>{card.desc}</p>

                  <ul className="space-y-2 mb-8">
                    {card.items.map(item => (
                      <li key={item} className={`flex items-center gap-2 text-xs ${card.dark ? 'text-white/40' : 'text-gray-400'}`}>
                        <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${card.dark ? 'bg-[#FFD447]' : 'bg-[#30358B]'}`} />
                        {item}
                      </li>
                    ))}
                  </ul>

                  <a href={card.href} className={`inline-flex items-center gap-2 text-sm font-bold group ${card.dark ? 'text-[#FFD447]' : 'text-[#0B0C1E]'}`}>
                    {card.cta}
        ...
        {/* ── HOW IT WORKS ── */}
        <section className="py-28 lg:py-36 bg-[#F7F8FC]">
          <div className="max-w-6xl mx-auto px-6 sm:px-10">

            <motion.div {...fadeUp} className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6 pb-10 mb-20 border-b border-black/[0.07]">
              <div>
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {[
                {
                  num: '01', title: 'Share Your Requirements',
                ...
                {
                  num: '03', title: 'Get Matches in 24 Hours',
                ...
                <motion.div
                  ...
                  {...stagger(i)}
                  {...lift}
                  className="relative bg-white rounded-2xl p-8 border border-black/[0.06] hover:border-[#30358B]/20 hover:shadow-2xl transition-all duration-300 group overflow-hidden"
                >
                  <div className="absolute -top-3 -right-2 text-[7rem] font-black text-black/[0.035] leading-none select-none">{step.num}</div>
                  ...
                  <h3 className="text-xl font-black text-[#0B0C1E] mb-3 leading-tight">{step.title}</h3>
            ...
            {!showSuccess ? (
              <motion.form {...fadeUp} onSubmit={handlePetFinderSubmit} className="space-y-10">
                ...
                <div>
                  <div className="flex items-center gap-3 mb-6 pb-4 border-b border-gray-100">
                    <div className="w-7 h-7 bg-[#FFD447] rounded-lg flex items-center justify-center">
                      <svg className="w-3.5 h-3.5 text-[#0B0C1E]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                      </svg>
                    </div>
                    <div>
                      <div className="text-sm font-bold text-[#0B0C1E]">Contact Information</div>
                      <div className="font-mono text-[9px] tracking-[0.1em] uppercase text-gray-400">We&apos;ll use this to send you pet matches</div>
                    </div>
                  </div>
                  <div className="grid md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <input type="text" name="city" value={formData.city} readOnly className="w-full px-4 py-3 rounded-xl border border-gray-100 bg-gray-50 text-gray-500 text-sm cursor-not-allowed" />
                ...
                <div>
                  <div className="flex items-center gap-3 mb-6 pb-4 border-b border-gray-100">
                    ...
                    <div>
                      ...
                      <div className="font-mono text-[9px] tracking-[0.1em] uppercase text-gray-400">Tell us what kind of pet you&apos;re looking for</div>
                    </div>
                  </div>
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-2.5">
                        {['child-friendly', 'active', 'calm', 'playful', 'guard dog', 'lap pet'].map(temp => (
                          <label key={temp} className={`flex items-center gap-2.5 px-3 py-3 rounded-xl border cursor-pointer transition-all duration-150 ${
                            formData.temperament.includes(temp) ? 'border-[#FFD447] bg-[#FFD447]/6 shadow-sm' : 'border-gray-100 hover:border-gray-200 bg-white'
                          }`}>
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        className="w-full px-4 py-3 rounded-xl border border-gray-100 text-sm focus:outline-none focus:ring-2 focus:ring-[#FFD447]/30 focus:border-[#FFD447]/50 transition-all resize-none"
                ...
                <div className="pt-2 space-y-3">
                  ...
                  >
                    {isSubmitting ? (<><Spinner />Finding your perfect pet…</>) : (
                      <>Get Matches in 24 Hours <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7l5 5m0 0l-5 5m5-5H6" /></svg></>
                    )}
                  </button>
              ...
              <motion.div {...fadeUp} className="py-20 text-center border border-black/[0.07] rounded-2xl">
                <div className="w-14 h-14 mx-auto mb-6 bg-emerald-100 rounded-full flex items-center justify-center">
        ...
        <section className="py-28 lg:py-36 bg-[#0B0C1E]">
          <div className="absolute inset-0 dot-grid opacity-30 pointer-events-none" style={{ position: 'relative' }} />
          <div className="max-w-6xl mx-auto px-6 sm:px-10">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
              {[
                { icon: <CheckCircle size={26} weight="bold" />, title: 'Verified Breeders', desc: 'Every breeder is screened for ethics, health standards, and proper care.' },
                { icon: <HeartStraight size={26} weight="fill" />, title: 'Health Guarantee', desc: 'Complete medical records, vaccination history, and health certificates.' },
                { icon: <ClockCounterClockwise size={26} weight="bold" />, title: '24-hr Match', desc: 'We find the right pet based on your lifestyle, preferences, and space.' },
                { icon: <Lock size={26} weight="fill" />, title: 'Secure Payments', desc: 'Protected transactions with transparent pricing and zero hidden fees.' },
              ].map((item, i) => (
                <motion.div
                  key={item.title}
                  {...stagger(i)}
                  {...lift}
                  className="group p-7 rounded-2xl bg-white/[0.04] border border-white/[0.07] hover:bg-white/[0.07] hover:border-[#FFD447]/20 transition-all duration-300"
                >
                  <div className="w-12 h-12 bg-[#FFD447]/10 rounded-xl flex items-center justify-center text-[#FFD447] mb-5 group-hover:bg-[#FFD447]/20 transition-colors duration-300">
                    {item.icon}
                  </div>
              ...
              <div className="flex flex-col gap-4 lg:items-end">
                ...
                <div className="inline-flex items-center bg-[#0B0C1E]/10 rounded-full p-1">
                  {(['dog', 'cat'] as const).map(type => (
                    <button key={type} onClick={() => setPlansPetType(type)} className={`px-6 py-2 rounded-full font-bold transition-all duration-200 flex items-center gap-2 text-xs ${plansPetType === type ? 'bg-[#0B0C1E] text-white shadow-lg' : 'text-[#0B0C1E]/50 hover:text-[#0B0C1E]'}`}>
                      {type === 'dog' ? <Dog size={14} weight="fill" /> : <Cat size={14} weight="fill" />}
                      {type === 'dog' ? 'Dogs' : 'Cats'}
                    </button>
                  ))}
                </div>
                ...
                {[
                  {
                    ...
                    desc: 'Affordable starter pack offering all core pet care needs.',
                  ...
                  <motion.div
                    ...
                    {...lift}
                    className={`relative flex flex-col rounded-2xl p-8 border transition-shadow duration-300 hover:shadow-2xl ${
                      plan.dark ? 'bg-[#0B0C1E] border-[#0B0C1E]' : 'bg-white border-black/[0.07]'
                    } ${plan.popular ? 'md:scale-[1.04]' : ''}`}
                  >
                    {plan.popular && (
                      <span className="absolute -top-3.5 left-1/2 -translate-x-1/2 px-4 py-1 bg-[#FFD447] text-[#0B0C1E] font-mono text-[9px] tracking-[0.2em] uppercase rounded-full shadow-lg">POPULAR</span>
                    )}
                    ...
                    <ul className="space-y-2.5 mb-8 flex-grow">
                      {plan.features.map(f => (
                        <li key={f} className={`flex items-start gap-2 text-xs ${plan.dark ? 'text-white/70' : 'text-gray-600'}`}>
                          <Check size={12} weight="bold" className={`flex-shrink-0 mt-0.5 ${plan.dark ? 'text-[#FFD447]' : 'text-emerald-500'}`} />
                          {f}
                        </li>
                      ))}
                    </ul>
              ...
              <div className="grid md:grid-cols-2 gap-5 max-w-3xl mx-auto">
                {[
                  ...
                  <motion.div
                    ...
                    {...stagger(i)}
                    {...lift}
                    className={`relative flex flex-col rounded-2xl p-8 border transition-shadow duration-300 hover:shadow-2xl ${plan.dark ? 'bg-[#0B0C1E] border-[#0B0C1E]' : 'bg-white border-black/[0.07]'}`}
                  >
                    {plan.popular && (
                      <span className="absolute -top-3.5 left-1/2 -translate-x-1/2 px-4 py-1 bg-[#FFD447] text-[#0B0C1E] font-mono text-[9px] tracking-[0.2em] uppercase rounded-full">POPULAR</span>
                    )}
                    <div className={`font-mono text-[9px] tracking-[0.2em] uppercase mb-3 ${plan.dark ? 'text-[#FFD447]' : 'text-[#30358B]'}`}>{plan.name}</div>
                    <p className={`text-xs mb-6 leading-relaxed ${plan.dark ? 'text-white/40' : 'text-gray-400'}`}>{plan.desc}</p>
                    <ul className="space-y-2.5 mb-8 flex-grow">
        ...
        <section className="py-28 lg:py-36 bg-[#F7F8FC]">
          <div className="max-w-6xl mx-auto px-6 sm:px-10">
            ...
            <div className="grid md:grid-cols-3 gap-5">
              {[
                { label: 'Food', desc: 'Premium nutrition for every life stage', icon: '🍖', color: 'bg-amber-50 border-amber-100' },
                { label: 'Accessories', desc: 'Toys, collars, beds & more', icon: '🎾', color: 'bg-emerald-50 border-emerald-100' },
                { label: 'Health', desc: 'Supplements, vitamins & care', icon: '💊', color: 'bg-sky-50 border-sky-100' },
              ].map((item, i) => (
                <motion.div
                  key={item.label}
                  {...stagger(i)}
                  {...lift}
                  className={`relative bg-white rounded-2xl p-8 border border-black/[0.06] hover:shadow-xl transition-all duration-300`}
                >
                  <span className="absolute top-5 right-5 font-mono text-[8px] tracking-[0.2em] uppercase px-2.5 py-1 bg-[#FFD447] text-[#0B0C1E]">Coming Soon</span>
                  <div className={`w-14 h-14 ${item.color} border rounded-2xl flex items-center justify-center text-3xl mb-5`}>{item.icon}</div>
        ...
        <section className="py-28 lg:py-36 bg-white">
          <div className="max-w-6xl mx-auto px-6 sm:px-10">
            ...
            <motion.div {...fadeUp} className="flex flex-col lg:flex-row lg:items-end gap-6 pb-10 mb-12 border-b border-black/[0.07]">
              <div>
            ...
            <div className="grid md:grid-cols-3 gap-5">
              {[
                { name: 'Priya Anandh', location: 'Edappaly, Kochi', text: 'Pet.Ra found us a Golden Retriever in 18 hours. The breeder was verified, and Bruno is healthy and happy!', initials: 'PA', dark: true },
                { name: 'Rohan Riju', location: 'Palarivattom, Kochi', text: 'Amazing service! Got 4 Persian kitten profiles with complete health records. Very professional team.', initials: 'RR', dark: false },
                { name: 'Max V', location: 'Koonammavu, Kochi', text: "As a first-time pet parent, Pet.Ra's guidance was invaluable. Found the perfect Beagle for our family!", initials: 'MV', dark: false },
              ].map((t, i) => (
                <motion.div
                  key={t.name}
                  {...stagger(i)}
                  {...lift}
                  className={`rounded-2xl p-7 border transition-shadow duration-300 hover:shadow-xl ${
                    t.dark ? 'bg-[#0B0C1E] border-white/[0.06]' : 'bg-white border-black/[0.07]'
                  }`}
                >
                  {/* Stars */}
                  <div className="flex gap-1 mb-4">
                    {Array.from({ length: 5 }).map((_, s) => (
                      <svg key={s} className="w-3.5 h-3.5 text-[#FFD447]" fill="currentColor" viewBox="0 0 20 20">
                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                      </svg>
                    ))}
                  </div>
                  <svg className={`w-7 h-7 mb-4 ${t.dark ? 'text-[#FFD447]/30' : 'text-[#FFD447]'}`} fill="currentColor" viewBox="0 0 24 24">
                    <path d="M14.017 21v-7.391c0-5.704 3.731-9.57 8.983-10.609l.995 2.151c-2.432.917-3.995 3.638-3.995 5.849h4v10h-9.983zm-14.017 0v-7.391c0-5.704 3.748-9.57 9-10.609l.996 2.151c-2.433.917-3.996 3.638-3.996 5.849h3.983v10h-9.983z" />
                  </svg>
                  <p className={`text-sm leading-relaxed mb-6 ${t.dark ? 'text-white/60' : 'text-gray-600'}`}>{t.text}</p>
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-black flex-shrink-0 ${t.dark ? 'bg-[#FFD447] text-[#0B0C1E]' : 'bg-[#0B0C1E] text-white'}`}>
                      {t.initials}
                    </div>
                    <div>
                      <div className={`text-sm font-black ${t.dark ? 'text-white' : 'text-[#0B0C1E]'}`}>{t.name}</div>
                      <div className={`font-mono text-[9px] tracking-[0.1em] uppercase mt-0.5 ${t.dark ? 'text-white/30' : 'text-black/30'}`}>{t.location}</div>
                    </div>
                  </div>
                </motion.div>
              ))}
              ...
              <div className="grid grid-cols-3 gap-10">
                ...
                <div>
                  ...
                  <div className="flex gap-4 mt-5">
                    <a href="https://instagram.com/thepetra.in" target="_blank" rel="noopener noreferrer" className="text-white/25 hover:text-white/70 transition-colors">
                    ...
                    <a href="https://www.linkedin.com/company/pet-ra/" target="_blank" rel="noopener noreferrer" className="text-white/25 hover:text-white/70 transition-colors">
        ...
        {showWaitlistModal && (
          <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setShowWaitlistModal(false)}>
            <motion.div
              ...
              className="bg-white max-w-md w-full shadow-2xl overflow-hidden rounded-2xl"
              onClick={e => e.stopPropagation()}
            >
              <div className="bg-[#0B0C1E] px-8 py-7 relative">
                <button onClick={() => setShowWaitlistModal(false)} className="absolute top-4 right-4 text-white/30 hover:text-white/70 transition-colors p-1.5 hover:bg-white/10 rounded-lg">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                </button>
                <div className="w-11 h-11 bg-[#FFD447] rounded-xl flex items-center justify-center mb-4">
                  <svg className="w-5 h-5 text-[#0B0C1E]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z" /></svg>
                </div>
                <div className="font-mono text-[9px] tracking-[0.3em] uppercase text-[#FFD447]/50 mb-2">Waitlist</div>
                <h3 className="text-xl font-black text-white">{selectedPlan} Plan</h3>
                <p className="font-mono text-[9px] tracking-widest uppercase text-white/30 mt-1">Early access pricing</p>
              </div>
              <form onSubmit={async e => {
                ...
                {[
                  ...
                  <div key={f.id}>
                    ...
                    <input type={f.type} required={f.req} value={f.val} onChange={e => setWaitlistData(prev => ({ ...prev, [f.key]: e.target.value }))} className="w-full px-4 py-3 rounded-xl border border-gray-100 text-sm focus:outline-none focus:ring-2 focus:ring-[#FFD447]/30 focus:border-[#FFD447]/50 transition-all" placeholder={f.ph} />
                  </div>
                ))}
                <button type="submit" disabled={isWaitlistSubmitting} className="w-full py-3.5 bg-[#FFD447] text-[#0B0C1E] font-bold rounded-full hover:bg-[#F5C800] transition-all disabled:opacity-50 flex items-center justify-center gap-2 text-sm">
                  {isWaitlistSubmitting ? <><Spinner />Joining…</> : 'Join Waitlist'}
                </button>
        ...
        {showProductModal && (
          <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setShowProductModal(false)}>
            ...
            >
              <div className="bg-[#0B0C1E] px-8 py-7 relative">
                <button onClick={() => setShowProductModal(false)} className="absolute top-4 right-4 text-white/30 hover:text-white/70 transition-colors p-1.5 hover:bg-white/10 rounded-lg">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                </button>
                <div className="w-11 h-11 bg-[#FFD447] rounded-xl flex items-center justify-center mb-4">
                  <svg className="w-5 h-5 text-[#0B0C1E]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" /></svg>
                </div>
                <div className="font-mono text-[9px] tracking-[0.3em] uppercase text-[#FFD447]/50 mb-2">Get Notified</div>
                <h3 className="text-xl font-black text-white">{selectedProduct} Products</h3>
                <p className="font-mono text-[9px] tracking-widest uppercase text-white/30 mt-1">Be first to know</p>
              </div>
              <form onSubmit={async e => {
                ...
                <div>
                  ...
                  <input type="email" required value={productNotifyEmail} onChange={e => setProductNotifyEmail(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-gray-100 text-sm focus:outline-none focus:ring-2 focus:ring-[#FFD447]/30 focus:border-[#FFD447]/50 transition-all" placeholder="your@email.com" />
                </div>
                <button type="submit" disabled={isProductNotifySubmitting} className="w-full py-3.5 bg-[#FFD447] text-[#0B0C1E] font-bold rounded-full hover:bg-[#F5C800] transition-all disabled:opacity-50 flex items-center justify-center gap-2 text-sm">
                  {isProductNotifySubmitting ? <><Spinner />Submitting…</> : 'Notify Me'}
                </button>
